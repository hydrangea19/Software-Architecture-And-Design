...
def generate_date_ranges(start_years_ago=10):
    ...
    start_date = end_date - relativedelta(
        years=start_years_ago
    )
    date_ranges = []

    current_start_date = start_date
    while current_start_date < end_date:
        current_end_date = current_start_date + relativedelta(
            years=1
        )
...
def get_last_saved_date_for_code_sync(code):
    return Issuer.objects.filter(code=code).order_by(
        '-date'
    ).first()


async def get_last_saved_date_for_code(code):
    last_saved = await get_last_saved_date_for_code_sync(
        code
    )
    return last_saved
    ...
    if isinstance(start, str):
        for date_format in ['%Y-%m-%d', '%d.%m.%Y']:
            try:
                ...
                continue
        raise ValueError(
            f"Invalid date string format: {start}"
        )
    if isinstance(start, date):
        return start
    raise TypeError(
        f"Unsupported type for start: {type(start)}"
    )
...
async def update_database_with_new_data(new_data):
    ...
    for record in new_data:
        ...
        if existing_record:
            print(
                f"Updating existing record for {record['code']} on "
                f"{record['date']}"
            )
            existing_record.code = record[
                'code'
            ]
            existing_record.date = record['date']
            existing_record.last_transaction_price = record[
            ...
            existing_record.percentage_change = record[
                'percent_change'
            ]
        ...
        if not existing_record:
            print(
                f"Creating existing record for {record['code']} on "
                f"{record['date']}"
            )
            new_entry = Issuer(
                code=record['code'],
                date=record['date'],
                last_transaction_price=record[
                    'last_transaction_price'
                ],
